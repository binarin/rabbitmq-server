...
listing_plugins_from_multiple_directories(Config) ->
    ...
    %% Everything was collected from both directories, plugin from first directory takes precendence.
    Path = FirstDir ++ ":" ++ SecondDir,
    Got = lists:sort([{Name, Vsn} || #plugin{name = Name, version = Vsn} <- rabbit_plugins:list(Path)]),
    Expected = [{plugin_both, "1"}, {plugin_first_dir, "3"}, {plugin_second_dir, "4"}],
    case Got of
        Expected ->
            ok;
        _ ->
